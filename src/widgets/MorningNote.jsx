    ...
    <div className="relative glass rounded-[16px] p-[28px_32px] overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 mb-[18px] flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => note ? generate(true) : generate()} disabled={loading || !hasData}
            className={clsx(
      ...
      {error && !loading && (
        <div className="border border-bear/30 bg-bear/5 rounded-xl p-4 space-y-2">
          ...
          <button onClick={() => generate(true)} className="mt-1 px-4 py-1.5 text-[12px] text-ts border border-bd rounded-lg hover:border-ts transition-colors cursor-pointer">Try again</button>
        </div>
      )}
        ...
        <>
          ...
          {meta && (
            <div className="flex items-center gap-4 mt-6 pt-4 border-t border-bd/40 text-[11px] text-tm flex-wrap">
              <span>{meta.model?.replace('gemini-', 'Gemini ') ?? 'Gemini'}{meta.cached ? ' · served from cache' : ''}</span>
