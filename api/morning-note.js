...
module.exports = async function(req, res) {
  ...
  const model  = 'gemini-1.5-flash'; // 15 RPM / 1,500 RPD — survived the Dec 2025 cuts intact
  ...
  try {
    ...
    if (r.status !== 200) {
      ...
      if (r.status === 429) return err(res, 'Gemini rate limit reached (gemini-1.5-flash free tier: 1,500 req/day). Try again in a moment.', 429);
