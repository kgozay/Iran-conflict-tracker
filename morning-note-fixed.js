 ...
 *
 * Uses Google Gemini 2.0 Flash for fast, high-quality sell-side brief formatting.
 * (Switched from 2.5 Pro: free tier is 1,500 RPD / 15 RPM vs 25 RPD / 5 RPM)
...
const NOTE_CACHE_TTL = 5 * 60 * 1000; // 5 minutes
let _cachedNote = null;
let _cacheTs    = 0;
let _cacheKey   = '';

function makeCacheKey(assets, sectors, cis) {
  // Key on CIS score + top asset prices rounded to 2dp — good enough to
  // detect a meaningful data refresh without serialising the whole payload.
  try {
    return JSON.stringify({
      cis:  cis?.total,
      b:    assets?.brent?.price   ? +assets.brent.price.toFixed(2)  : null,
      g:    assets?.gold?.price    ? +assets.gold.price.toFixed(0)   : null,
      z:    assets?.usdZar?.price  ? +assets.usdZar.price.toFixed(3) : null,
    });
  } catch { return ''; }
}
...
module.exports = async function(req, res) {
  ...
  /* ── Cache check ─────────────────────────────────────────────── */
  const cacheKey = makeCacheKey(assets, sectors, cis);
  const now      = Date.now();
  if (_cachedNote && cacheKey === _cacheKey && (now - _cacheTs) < NOTE_CACHE_TTL) {
    console.log('[morning-note] Serving cached note (age:', Math.round((now - _cacheTs) / 1000), 's)');
    return ok(res, { ..._cachedNote, cached: true });
  }

  const prompt = buildPrompt(assets, sectors, cis, stocks || [], alerts || [], dataHealth || null);
  // ↓ Switched from gemini-2.5-pro (25 RPD free) → gemini-2.0-flash (1,500 RPD free)
  const model  = 'gemini-2.0-flash';

  console.log('[morning-note] Calling Gemini API (' + model + ')…');

  /* ── Call Gemini with 1 retry on 429 ────────────────────────── */
  ...
  try {
    let resHttp = await callGemini();

    // 1 auto-retry after 3s on rate limit
    if (resHttp.status === 429) {
      console.warn('[morning-note] 429 on first attempt — retrying in 3s…');
      await sleep(3000);
      resHttp = await callGemini();
    }
    ...
    if (resHttp.status !== 200) {
      ...
      if (resHttp.status === 429) {
        return err(res,
          'Gemini rate limit reached. You are on the free tier (1,500 req/day). ' +
          'Wait a minute and try again, or upgrade at aistudio.google.com.',
          429
        );
      }
